...
import { apiGetBalance, apiGetCurrentUser, apiGetReceipts, apiGetRedemptionOrders } from "@/lib/api/client";
...
export default async function AccountPage() {
  ...
  const [{ balance }, receipts, orders] = await Promise.all([
    apiGetBalance(),
    apiGetReceipts(),
    apiGetRedemptionOrders(),
  ]);
            ...
            {orders.slice(0, 10).map((o) => (
              <li key={o.id} className="flex flex-col gap-1 px-4 py-3 text-sm">
                ...
                <span className="text-xs text-zinc-500">
                  {o.items.map((item) => `${item.quantity}x ${item.gift?.name ?? "Unknown gift"}`).join(", ")}
                </span>
                {o.trackingNumber && (
                  <span className="text-xs text-blue-600">Tracking: {o.trackingNumber}</span>
                )}
              </li>
            ))}
          </ul>
