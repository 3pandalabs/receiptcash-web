...
export default async function AccountPage() {
  ...
  const [{ data: balanceRow }, { data: receipts }, { data: redemptions }] = await Promise.all([
    ...
    supabase
      .from("redemptions")
      .select("id, points_cost, status, created_at, gifts(name)")
      .order("created_at", { ascending: false })
      .limit(10),
  ]);
          ...
          <ul className="divide-y divide-zinc-200 rounded-xl border border-zinc-200 dark:divide-zinc-800 dark:border-zinc-800">
            {receipts.map((r) => (
              <li key={r.id} className="flex items-center justify-between px-4 py-3 text-sm">
                <span>{r.merchant_name ?? "Processing..."}</span>
      ...
      <section>
        ...
        {redemptions && redemptions.length > 0 ? (
          <ul className="divide-y divide-zinc-200 rounded-xl border border-zinc-200 dark:divide-zinc-800 dark:border-zinc-800">
            {redemptions.map((r) => (
              <li key={r.id} className="flex items-center justify-between px-4 py-3 text-sm">
                <span>{(r.gifts as unknown as { name: string } | null)?.name ?? "Gift"}</span>
                <span className="flex items-center gap-3">
